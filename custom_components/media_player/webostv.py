...
SERVICE_CLOSE_WEB = 'close_web'
SERVICE_OPEN_YOUTUBE = 'open_youtube'
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    setup_tv(host, mac, name, customize, config, timeout, hass, add_devices, on_action)


def setup_tv(host, mac, name, customize, config, timeout, hass, add_devices, on_action):
    ...
    def open_youtube(call):
        video_id = call.data.get('video_id')
        client.request('system.launcher/launch', {'id': 'youtube.leanback.v4', 'contentId': video_id})
    
    hass.services.register(DOMAIN, SERVICE_OPEN_URL + '_' + host.replace('.', '_'), open_url)
    hass.services.register(DOMAIN, SERVICE_CLOSE_WEB + '_' + host.replace('.', '_'), close_web)
    
    hass.services.register(DOMAIN, SERVICE_OPEN_YOUTUBE + '_' + host.replace('.', '_'), open_youtube)
    
    add_devices([LgWebOSDevice(host, mac, name, customize, config, timeout, on_action, hass)], True)
...
class LgWebOSDevice(MediaPlayerDevice):
    ...
    def __init__(self, host, mac, name, customize, config, timeout, on_action, hass):
    ...
    def update(self):
        ...
        try:
            ...
            if self._state is not STATE_OFF:
                ...
                """_LOGGER.error(self._client.get_channel_info())"""
                
                channel_info = self._client.get_channel_info()
                if channel_info is not None:
                    self._media_title = channel_info['channel']['channelName'] if channel_info['returnValue'] is True else None
                else:
    ...
    def supported_features(self):
        ...
        if self._mac or self._on_script is not None:
    ...
    def turn_on(self):
        ...
        elif self._on_script is not None:
            self._on_script.run()
